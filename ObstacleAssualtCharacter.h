...
struct FLedgeInfo
{
	...
	UPROPERTY() float   LedgeHeightWorld = 0.f;                   // 상면 월드 Z(디버그)
	UPROPERTY() AActor* HitActor = nullptr;               // 맞은 액터(옵션)
...
protected:
	...
	FLedgeInfo CurrentLedge; // ← 헤더에 멤버로 사용 ok (정의가 위에 있으므로)
	...
	UFUNCTION() // ← 반드시 필요
		void OnCapsuleHit(
			...
			const FHitResult& Hit   // ← const FHitResult& 여야 함
		);
	/** Jump Input Action */
...
protected:
	...
	/** 2D 음악을 관리할 오디오 컴포넌트 (BeginPlay에서 Spawn) */
	UPROPERTY(Transient)
	...
	/** 보간 속도 (값/초) */
	UPROPERTY(EditDefaultsOnly, Category = "PostProcess")
	float DesatInterpSpeed = 6.0f; // 빠르게 0.15~0.25초 정도

	/** 위젯 BP 클래스 (WBP_Playtime 지정) */
	...
	// 루트모션 사용 여부(몽타주가 루트모션 포함이면 true 권장)
	UPROPERTY(EditDefaultsOnly, Category = "Ledge|Anim")
